...
public class SitEntUimaUtils {
	 ...
	 * @return
	 */
	public static NonEmptyFSList addToFSList(FSList fsList, TOP newItem,
			JCas jcas) {
	 ...
	 * @return
	 */
	public static NonEmptyStringList addToStringList(StringList sList,
			String newString, JCas jcas) {
	...
	public static boolean covers(Annotation covering, Annotation covered) {
		if (covering.getBegin() <= covered.getBegin()
				&& covering.getEnd() >= covered.getEnd()) {
			return true;
		}
		return false;
	}
	...
	public static boolean covers(Annotation covering, Annotation covered,
			JCas jcas) {
		List<? extends Annotation> allCovered = JCasUtil.selectCovering(jcas,
				covered.getClass(), covering);
	...
	public static boolean overlapsDirected(Annotation annot1, Annotation annot2) {
		if (annot1.getBegin() <= annot2.getBegin()
	 ...
	 * @return
	 */
	public static boolean overlaps(Annotation annot1, Annotation annot2,
			JCas jCas) {
		if (overlapsDirected(annot1, annot2)
				|| overlapsDirected(annot2, annot1)
				|| covers(annot1, annot2, jCas) || covers(annot2, annot1, jCas)) {
			return true;
		}
		return false;
	}
